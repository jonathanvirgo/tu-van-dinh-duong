var express         = require('express'),
    ...
    modelService    = require('../models/menuExampleModel'); 
...
router.get('/', function (req, res, next) {
    try {
        ...
        if(!logService.authorizeAccess(req.user.role_id, 'menu-example')){
...
router.get('/edit/:id', function (req, res) {
    try {
        ...
        modelService.getMenuExampleById(req.params.id, function (err, result, fields) {
            if (err) {
                adminService.addToLog(req, res, err);
                return;
            }
            if(result[0] == undefined){
                adminService.addToLog(req, res, 'Không tìm thấy thực đơn mẫu nào có id=' + req.params.id);
                return;
            }
            res.render(viewPage("edit"), {
                user: req.user,
                menuExample: result[0],
                errors: []
            });
        })
    } catch (e) {
...
router.post('/edit/:id', function (req, res, next) {
    try {
        ...
        if(parameter.name == ''){
            str_error.push("Thiếu tên thực đơn mẫu!");
        }
        if(str_error.length > 0){
            res.render(viewPage("edit"), {
                user: req.user,
...
router.post('/list', function (req, res, next) {
    ...
    try {
        ...
        arrPromise.push(new Promise(function (resolve, reject) {
            modelService.countAllMenuExample(parameter, function (err, result, fields) {
                if (err) {
        ...
        arrPromise.push(new Promise(function (resolve, reject) {
            modelService.getAllMenuExampleFromParam(parameter, function (err, result, fields) {
                if (err) {
                    return logService.create(req, err).then(function(responseData){
                        if(responseData.message) resultMessage.error = responseData.message;
                        else resultMessage.error = err.sqlMessage;
...
function viewPage(name) {
    return path.resolve(__dirname, "../views/menu_example/" + name + ".ejs");
}
