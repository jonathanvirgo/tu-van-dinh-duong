...
router.get('/', function (req, res, next) {
    try {
        ...
        if(!logService.authorizeAccess(req.user.role_id, 'menu-time')){
...
router.get('/edit/:id', function (req, res) {
    try {
        ...
        modelService.getMenuTimeById(req.params.id, function (err, result, fields) {
            if (err) {
...
router.post('/create', function (req, res, next) {
    try {
        ...
        if(parameter.name == ''){
            str_error.push("Thiếu tên giờ ăn!");
        }
    ...
    try {
        ...
        modelService.delete(record_id, function (err, results, fields) {
            ...
            if(affectedRow > 0){
                ...
                adminService.addToLog(req, res, 'Không tìm thấy giờ ăn nào có id=' + req.params.id);
            }
        })
    } catch (e) {
        adminService.addToLog(req, res, e.message);
...
router.post('/list', function (req, res, next) {
    ...
    try {
        ...
        arrPromise.push(new Promise(function (resolve, reject) {
            modelService.countAllMenuTime(parameter, function (err, result, fields) {
                if (err) {
        ...
        arrPromise.push(new Promise(function (resolve, reject) {
            modelService.getAllMenuTime(parameter, function (err, result, fields) {
                if (err) {
                    return logService.create(req, err).then(function(responseData){
                        if(responseData.message) resultMessage.error = responseData.message;
                        else resultMessage.error = err.sqlMessage;
