...
let medicineService = {
    create: function (parameter, callback) {
        db.get().getConnection(function (err, connection) {
            try {
                ...
                var sql   = "INSERT INTO medicine (name,unit,description,hospital_id,department_id,created_by) VALUES (?,?,?,?,?,?)";
                var query = connection.query(sql, [parameter.name,parameter.unit,parameter.description,parameter.hospital_id,parameter.department_id,parameter.created_by], function (err, results, fields) {
                    connection.release();
                    if (err) return callback(err);
    ...
    update: function (parameter, callback) {
        db.get().getConnection(function (err, connection) {
            try {
                if (err) return callback(err);
                var sql   = 'UPDATE medicine SET name = ?, unit = ?, description = ?, hospital_id = ?, department_id = ?, created_by = ? WHERE id=?';
                var query = connection.query(sql, [parameter.name,parameter.unit,parameter.description, parameter.hospital_id, parameter.department_id, parameter.created_by,  parameter.id], function (err, results, fields) {
                    connection.release();
                    if (err) return callback(err);
                    callback(null, results, fields);
                });
